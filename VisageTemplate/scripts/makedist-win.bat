...
if %DEMO_ARG% == "demo" (
  echo Making VisageTemplate Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making VisageTemplate Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild VisageTemplate-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild VisageTemplate.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild VisageTemplate.sln /t:VisageTemplate-app;VisageTemplate-vst3;VisageTemplate-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\VisageTemplate.aaxplugin\Contents\Win32\VisageTemplate.aaxplugin --out .\build-win\aax\bin\VisageTemplate.aaxplugin\Contents\Win32\VisageTemplate.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\VisageTemplate.aaxplugin\Contents\x64\VisageTemplate.aaxplugin --out .\build-win\aax\bin\VisageTemplate.aaxplugin\Contents\x64\VisageTemplate.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\VisageTemplate.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\VisageTemplate.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "VisageTemplate Installer" ".\installer\VisageTemplate Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\VisageTemplate Installer.exe" ".\installer\VisageTemplate Demo Installer.exe"
  REM -del ".\installer\VisageTemplate Installer.exe"
  REM -)
