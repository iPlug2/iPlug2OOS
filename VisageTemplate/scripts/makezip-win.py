...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\VisageTemplate Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\VisageTemplate Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\VisageTemplate manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\VisageTemplate.vst3\\Contents\\x86_64-win\\VisageTemplate.vst3",
      projectpath + "\\build-win\\VisageTemplate_x64.exe",
      projectpath + "\\build-win\\VisageTemplate.clap"
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\VisageTemplate-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\VisageTemplate-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\VisageTemplate-clap_x64.pdb"
  ]
