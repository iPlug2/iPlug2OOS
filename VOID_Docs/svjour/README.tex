...
QUERY = """
Given this header file, update it so 
that it also includes a controllable 
parameter for mixAmount and width
"""
ASSOCIATED = ""
\end{verbatim}
...
QUERY = """
Given this cpp file, update it so 
that it also includes the 
controllable parameter for mixAmount 
and width specified in 
"""
ASSOCIATED = "TemplateProject/TemplateProject.h"
\end{verbatim}

The files that it overwrites are stored at \texttt{agent\_backups/} and can all be restored using:
\begin{lstlisting}
./restore.py
\end{lstlisting}
...
Now includes scripts to setup LaTeX to compile the README.tex file. Edits can be made to the PDF by editing the file at \texttt{VOID\_Docs/svjour/README.tex} and compiling. When you are ready to compile them, first use:
\begin{lstlisting}
